...
namespace page
{
	namespace aud
	{
		...
		// construct
		SpacialChannel::SpacialChannel(const phys::Sound &sound) :
			Channel(sound.GetPlayPosition()), id(sound.GetId()),
			occluded(false), level(1) {}

		// state
		bool SpacialChannel::IsAudible() const
		{
			return level;
		}

		// update
		void SpacialChannel::DoUpdate(float deltaTime)
		{
			if (occluded)
		...
		// modifiers
		void SpacialChannel::Occlude()
		{
			occluded = true;
		}

		// sound access
		phys::Sound *SpacialChannel::GetSound() const
		{
			return util::PointerFromId<phys::Sound>(id);
		}
		util::Identifiable::Id SpacialChannel::GetId() const
		{
			return id;
		}

		// state
		float SpacialChannel::DoGetLinearVolume() const
		{

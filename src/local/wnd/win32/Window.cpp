...
namespace page { namespace wnd { namespace win32
{
	...
	LRESULT Window::WindowProc(UINT msg, WPARAM wparam, LPARAM lparam)
	{
		switch (msg)
		{
			...
			{
				math::Vec2i newPos(GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam));
			...
			{
				math::Vec2u newSize(LOWORD(lparam), HIWORD(lparam));
			...
			if (alive && !min)
			{
				if (moving)
				{
					...
					GetClientRect(hwnd, &rect);
					MapWindowPoints(hwnd, 0, reinterpret_cast<LPPOINT>(&rect), 1);
					moveSig(*(CVAR(windowPosition) = math::Vec2i(rect.left, rect.top)));
					moving = false;
				}
				if (sizing)
				{
					RECT rect;
					GetClientRect(hwnd, &rect);
					sizeSig(*(CVAR(windowSize) = math::Vec2u(rect.right, rect.bottom)));
					sizing = false;
				}
			}
			break;

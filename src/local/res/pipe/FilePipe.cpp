...
namespace page
{
	namespace res
	{
		namespace
		{
			...
			{
				if (!fs)
					if (sys::IsFile(path)) THROW err::FileAccessException<err::ResourceTag>();
					else THROW err::FileNotFoundException<err::ResourceTag>();
				// determine file size
			...
			void FileStream::DoRead(void *s, unsigned n)
			{
				...
				if (!fs.read(static_cast<char *>(s), n))
					if (fs.eof()) THROW err::EndOfFileException<err::ResourceTag>();
					else THROW err::FileReadException<err::ResourceTag>();
			}
			unsigned FileStream::DoReadSome(void *s, unsigned n)
			{
				if (!fs) fs.clear();
				if (fs.read(static_cast<char *>(s), n).bad())
					THROW err::FileReadException<err::ResourceTag>();
				return fs.gcount();
			}
			...
			unsigned FileStream::DoTell() const
			{
				...
				if (pos == std::ifstream::pos_type(-1))
					THROW err::FileReadException<err::ResourceTag>("file tell error");
				return pos;
			}
			...
			void FileStream::DoSeek(unsigned n)
			{
				...
				if (!fs.seekg(n))
					if (fs.eof()) THROW err::EndOfFileException<err::ResourceTag>();
					else THROW err::FileReadException<err::ResourceTag>("file seek error");
			}
		}

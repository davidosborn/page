...
namespace page
{
	namespace res
	{
		namespace
		{
			struct FlacStream : AudioStream
			{
				// construct
				FlacStream(const Pipe &);
			...
			FlacStream::FlacStream(const Pipe &pipe) : init(false),
				sd(Open(pipe, std::bind(&FlacStream::Write, this,
					...
					std::placeholders::_3)))
			{
				if (!sd) THROW err::PlatformException<err::FlacPlatformTag, err::ResourceTag, typename err::FormatException<>::Tags>(err::FormatException<>().What());
			}
		...
		// operations
		AudioStream *FlacDecoder::Open() const
		{
			return new FlacStream(*pipe);
		}

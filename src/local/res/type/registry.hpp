...
#ifndef    page_res_type_registry_hpp
#   define page_res_type_registry_hpp
...
namespace page
{
	namespace res
	{
		// access
		/**
		 * Returns the registered name of a given resource type.
		 */
		const std::string &GetRegisteredTypeName(const std::type_info &);
		/**
		 * Returns the registered deleter of a given resource type.
		 */
		const util::Deleter &GetRegisteredTypeDeleter(const std::type_info &);
		/**
		 * Returns the registered post-loader of a given resource type.
		 */
		const PostLoadFunction &GetRegisteredTypePostLoader(const std::type_info &);

		// registration
		/**
		 * Registers a resource type with the system.
		 */

	...
	namespace res
	{
		Mesh *LoadNativeMesh(const std::shared_ptr<const Pipe> &pipe)
		{
			...
			for (std::vector<fmt::Vertex>::const_iterator fmtVertex(vertices.begin()); fmtVertex != vertices.end(); ++fmtVertex)
			{
				Mesh::Vertex vertex =
				{
					math::Vector<3>(fmtVertex->co[0], fmtVertex->co[1], fmtVertex->co[2]),
					math::Vector<3>(fmtVertex->no[0], fmtVertex->no[1], fmtVertex->no[2]),
					math::Vector<2>(fmtVertex->uv[0], fmtVertex->uv[1]),
					math::Vector<2>(), // FIXME: texture coordinate 2 goes here
					math::Vector<2>()  // FIXME: texture coordinate 3 goes here
				};
				// fill influences

...
namespace page
{
	...
	{
		// construct
		DirSource::DirSource(const std::string &path) : path(path)
		{
			sys::WalkDir(path, std::bind(&DirSource::IndexFile, this, std::placeholders::_1));
		}

		// modifiers
		void DirSource::Refresh()
		{
		...
		// indexing
		void DirSource::IndexFile(const std::string &path)
		{
		...
		// factory function
		Source *MakeDirSource(const std::string &path)
		{
			std::string absPath(sys::AbsPath(sys::NormPath(path)));
			return sys::IsDir(absPath) ? new DirSource(absPath) : 0;
		}

		REGISTER_SOURCE(MakeDirSource, 0)
	}
}

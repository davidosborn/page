...
namespace page
{
	namespace res
	{
		...
		// factory function
		Source *MakeZipSource(const std::string &path)
		{
			std::string absPath(sys::AbsPath(sys::NormPath(path)));
			// check signature
			std::ifstream fs(absPath.c_str(), std::ios_base::binary | std::ios_base::in);
			if (!fs) return 0;
			char sig[sizeof fmt::sig];
			if (fs.read(sig, sizeof sig).bad() || fs.gcount() != sizeof sig) return 0;
			return new ZipSource(absPath);
		}

		REGISTER_SOURCE(MakeZipSource, 50)
	}
}

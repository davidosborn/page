...
#include <algorithm> // any_of, copy{,_n}, equal, fill_n, max, swap_ranges, transform
#include <cassert>
...
namespace page { namespace math
{
	...
	// zero equivalence
	template <typename T> Quat<T>::operator bool() const
	{
		return std::any_of(begin(), end(), [](T t) { return t; });
	}
	...
	{
		return Vec3(q * Quat<T>(v) * Inv(q));
	}

	// stream insertion/extraction
	template <typename Char, typename CharTraits, typename T>
		std::basic_ostream<Char, CharTraits> &operator <<(std::basic_ostream<Char, CharTraits> &os, const Quat<T> &q)
	{
	...
	template <typename Char, typename CharTraits, typename T>
		std::basic_istream<Char, CharTraits> &operator >>(std::basic_istream<Char, CharTraits> &is, Quat<T> &q)
	{

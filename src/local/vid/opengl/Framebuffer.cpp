...
namespace page
{
	namespace vid
	{
		...
		{
			// construct/destroy
			Framebuffer::Framebuffer(const math::Vec2u &size, GLenum colorFormat, Flags flags) :
			...
			// attributes
			const math::Vec2u &Framebuffer::GetSize() const
			{
				return size;
			}
			const math::Vec2u &Framebuffer::GetPow2Size() const
			{
				return pow2Size;
			}
			...
			void Bind(const Framebuffer &fb)
			{
				...
				math::Vec2u size(fb.GetSize());
				glViewport(0, 0, size.x, size.y);
			}
			void ResetFramebufferBinding(const DrawContext &dc)
			{
				glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, 0);
				math::Vec2u size(dc.GetDriver().GetWindow().GetSize());
				glViewport(0, 0, size.x, size.y);
			}
		}
	}

...
namespace page
{
	namespace phys
	{
		...
		{
			// construct
			PositionOrientation::PositionOrientation(const math::Vector<3> &position, const math::Quat<> &orientation) :
				Position(position), Orientation(orientation) {}
			PositionOrientation::PositionOrientation(const math::Vector<3> &position, const math::Vector<3> &normal) :
				Position(position), Orientation(normal) {}
			PositionOrientation::PositionOrientation(const math::Matrix<3, 4> &matrix) :
				Position(matrix), Orientation(math::Matrix<3>(matrix)) {}
			...
			void PositionOrientation::SetMatrix(const math::Matrix<3, 4> &matrix)
			{
				...
				Orientation::SetMatrix(math::Matrix<3>(matrix));
			}

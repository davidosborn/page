...
namespace page
{
	namespace phys
	{
		...
		{
			// construct
			Position::Position(const math::Vector<3> &position) :
			...
			// access
			const math::Vector<3> &Position::GetPosition() const
			{
				return position;
			}
			void Position::SetPosition(const math::Vector<3> &position)
			{
			...
			// transform state
			const math::Vector<3> &Position::GetLastPosition() const
			{
				return lastPosition;
			}
			const math::Vector<3> &Position::GetForce() const
			{
				return force;
			}
			const math::Vector<3> &Position::GetVelocity() const
			{
				return velocity;
			}

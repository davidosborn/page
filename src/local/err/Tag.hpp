...
////////////////////////////////////////////////////////////////////////////////
	...
	namespace detail
	{
		 ...
		 *
		 * @note Designed to be use with @c boost::mpl::sort to sort a sequence
		 *       of tags.
		 */
		struct LessTags
		{
		...
		/**
		 * A binary metafunction implementing an equivalence relation for tags.
		 *
		...
		/**
		 * Recursively expands nested sequences of tags to produce a single flat
		 * sequence of tags.
		 *
		 * @note The outer sequence is represented by a Boost/MPL sequence,
		 *       while nested sequences are represented by types derived from
		 *       @c TagSet.
		 */
		...
		/**
		 * Process a variadic sequence of @c Tag and @c TagSet types to produce
		 * a flattened and sorted Boost/MPL sequence of distinct tags.
		 */
		template <typename... Tags>
			struct CleanTags
		{
...
////////////////////////////////////////////////////////////////////////////////
	...
	/**
	 * A function object which takes a tag, converts it to a @c RuntimeTag, and
	 * writes it to an output iterator.
	 *
	...
	{
		/**
		 * @note We can't use @c util::Serialize here because it would cause an
		 *       @c #include dependency-cycle.
		 */
		std::copy(
			tagSet.tags.begin(),
			tagSet.tags.end(),
